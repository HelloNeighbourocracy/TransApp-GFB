...
- **Live transcript revisions:** Chrome's speech engine often re-fires a growing version of the same sentence multiple times as it processes more audio, rather than one clean final result. The app collapses those into a single updated line by tagging each spoken utterance with a stable id and matching translation responses back to that id -- so even if two translation requests resolve out of order (a real possibility over the network), each one updates the correct line instead of clobbering a different one. If you still occasionally see two very similar lines close together, it usually means the engine paused briefly (4+ seconds) between them and treated it as a genuinely new sentence.
- **Transcript/PDF labels:** earlier versions tagged each line "[English]" / "[Tamil]" -- English words, which defeats the point for a reader who doesn't read English. Lines are no longer tagged; the original is shown smaller/lighter and the translation larger/bold (same convention as the on-screen log), and the one header that does need a language label uses each language's own native script for its own name.
