...
function MainApp({ user, isPro, trialLangs, onLogout, daysLeft, expiry, onChangePw }) {
  ...
  const utteranceIdRef = useRef(0)    // global counter — never resets
  const latestRequestRef = useRef(0)
  const transcriptsRef = useRef([])   // mirror for download access

  useEffect(() => {
    sourceLangRef.current = sourceLang
    // If meeting is running, restart recognizer with new lang (or no lang for auto)
    if (shouldRunRef.current && recognizerRef.current) {
      recognizerRef.current.stop()
      // onend handler will restart with updated sourceLangRef
    }
  }, [sourceLang])
  ...
  const handleFinal = async (text) => {
    ...
    setTranscripts((prev) => {
      ...
      let next
      if (idx !== -1) {
        next = [...prev]; next[idx] = entry
      } else {
        next = [...prev, entry]
      }
      transcriptsRef.current = next  // keep ref in sync for download
      return next
    })
  ...
  const start = () => {
    ...
    // utteranceIdRef: do NOT reset — keeps transcript entries in correct order
    ...
    <div className="min-h-screen flex flex-col items-center px-4 py-10 md:py-16 font-body">
      ...
      <main className="console-panel w-full max-w-3xl p-6 md:p-10">
        ...
        <ControlDeck
          ...
          onStop={stop}
          subtitle={subtitle}
          onDownload={() => downloadTranscript(transcriptsRef.current, sourceLang, targetLang)}
